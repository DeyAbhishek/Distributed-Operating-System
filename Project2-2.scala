...
object project2bonus {
 ...
 val sys=  ActorSystem("MySystem")
  val s= Array
  var fail_nodes=0
  var Failure:Array[Int]=null
  def main(args: Array[String]) {
	  	val noOfActors= args(0).toInt
	  			val topology= args(1)
	  			val algorithm= args(2)
	  			fail_nodes= args(3).toInt
	  			Failure= new Array[Int](fail_nodes)
  ...
  class Master(noOfActors:Int,topology:String,Algorithm:String) extends Actor
  {
    ...
    for(w<-0 to fail_nodes-1)
    {
      var ravan=Random.nextInt(noOfActors)
      Failure(w)=ravan
    }
	  	val random = Random.nextInt(noOfActors)
    ...
    def receive =  {
      ...
      case CONVERGE=>
       ...
       // print("  Position:"+count1)
          if(count1==(noOfActors-fail_nodes))
          {
             var endTime=System.currentTimeMillis()
